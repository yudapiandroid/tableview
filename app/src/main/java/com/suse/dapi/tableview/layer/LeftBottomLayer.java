...
public class LeftBottomLayer implements DrawLayer {
    ...
    private void drawCell(FourPerCellModel m, Rect rect, ICanvasGL canvas) {
        ...
        canvas.drawCircle(rect.left+width / 2, 30, 30, circlePaint);

        if(m.isDrawLT()){
            int rx = rect.left + width / 4 + padding;
            int ry = rect.top + height / 4 + padding;
            int r = width / 4;
            glPaint.setColor(m.getColorLT());
            glPaint.setLineWidth(4);
        ...
        if(m.isDrawRT()){
            ...
            int ry = rect.top + height / 4 + padding;
            int r = width / 4;
            glPaint.setColor(m.getColorRT());
            glPaint.setLineWidth(2);
            glPaint.setStyle(Paint.Style.STROKE);
            canvas.drawCircle(rx ,ry ,r,glPaint);
        }
